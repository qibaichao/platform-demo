...
<div class="right_botoom" id="list-page">
    <table width="768" border="0" cellspacing="0" cellpadding="0">
      ...
      <#if pager.resultList??>
		<#list pager.resultList as userVo>
			<tr>
			<td>
				<input name="ids" id="ids"  type="checkbox" value="${userVo.userId}">
			</td>
			<td>${userVo.userId}</td>
			...
			<td>${userVo.age}</td>
			<td>
			 <@p.linkpage value=pager uri='showUserModify.aj' param="${userVo.userId}">修改</@p.linkpage>
             <@p.linkpage value=pager uri='userLook.aj'   param='${userVo.userId!}' >查看</@p.linkpage>
            </td>
			</tr>
		</#list>
	</#if>				
